...
//                    }
            ...
            for (int j = 0; j <= day.length; j++) {
                for (int i = 0; i <= time.length; i++) {
                    if (value[0][i].startsWith(practical) && (i+1)<time.length) {
...
//        boolean finalOp = false;
        ...
        boolean itration_Analyzer = false;
        while (!itration_Analyzer){
            itration_Analyzer = true;
                            ...
                            for (int i = 1; i <= time.length; i++) {
                                if (value[j][i].contains(("B1")) || value[j][i].contains(("B2")) || value[j][i].contains(("B3")) || value[j][i].contains(("B4"))) {
                                    ...
                                    for (int k = 0; k < TotalSubcode.length; k++) {
                                        ...
                                        if (!(TotalSubcode[k].equals("")) && !value2[j][i].contains("B1")) {
                                            String subcode = TotalSubcode[k];
                                            subcode = TotalSubcode[k].substring(0, TotalSubcode[k].length() - 2);
                                            String subcodeTeacher1 = TeacherNSubject.get(subcode);
                                            String subcodeTeacher2 = TeacherNSubject.get(value2[j][i]);
                                           boolean itration = false;
                                            if (subcodeTeacher1.equals(subcodeTeacher2) && subcodeTeacher1.trim().length()>0) {
//                                             finalOp = false;
                                                while (!itration) {
                                                    itration_Analyzer = false;
                                                    int dayint = Randome(1, day.length - 1);
                                                    int timeint = Randome(1, time.length - 1);
                                                    ...
                                                    if (suffel1.contains(("B1")) || suffel1.contains(("B2")) || suffel1.contains(("B3")) || suffel1.contains(("B4"))) {
                                                        ...
                                                        value2[j][i] = suffel1;
//                                                    System.out.println(value2[j][i]);

                                                        if (value2[j][i].toString().trim().equals("----")) {

                                                        } else {
//                                                        System.out.println(TeacherNSubject.get(value2[j][i])+"==="+subcodeTeacher1);
...
//                                                             System.out.println("true");
                                            ...
                                            } else if (subcodeTeacher2 != null & subcodeTeacher1 != null) {
//                                            loop++;
                                            }
                                        }
                                    }
